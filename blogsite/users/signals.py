from django.db.models.signals import post_save, pre_save
from django.contrib.auth.models import User
from django.dispatch import receiver
from .models import Profile, Authentication
from django.contrib.auth import logout

# from django import dispatch

# # Define a custom registration signal
# registration_signal = dispatch.Signal()


@receiver(post_save, sender=User)
def create_profile(sender, instance, created, **kwargs):
    if created:
        Profile.objects.create(user=instance)


@receiver(post_save, sender=User)
def save_profile(sender, instance, **kwargs):
    instance.profile.save()

# @receiver(post_save, sender=User)
# def create_authentication(sender, instance, created, **kwargs):
#     if created:
#         Authentication.objects.create(user=instance)

# @receiver(post_save, sender=User)
# def save_authentication(sender, instance, **kwargs):
#     if hasattr(instance, 'authentication'):
#         instance.authentication.save()


